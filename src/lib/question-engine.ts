...
function convertQuantToVerified(q: QuestionItem): VerifiedQuestionItem {
  return {
    ...
    solution: {
      ...
      commonMistakeTrap: q.commonTrap,
    },
  };
}
...
export function getDynamicPracticeQuestions(params: {
  ...
  // Filter by topic / track if specified
  if (targetTopic && targetTopic !== "all" && targetTopic !== "diagnostic" && targetTopic !== "daily-challenge" && targetTopic !== "weak-areas") {
    ...
    if (targetTrack === "qa" || targetTrack === "quant") {
      candidatePool = candidatePool.filter((q) => !q.id.startsWith("dilr-") && !q.id.startsWith("varc-") && !q.id.startsWith("special-"));
    } else if (targetTrack === "dilr") {
      candidatePool = candidatePool.filter((q) => q.id.startsWith("dilr-") || q.topicSlug.includes("arrangements") || q.topicSlug.includes("logic"));
    } else if (targetTrack === "varc") {
      candidatePool = candidatePool.filter((q) => q.id.startsWith("varc-") || q.topicSlug.includes("rc") || q.topicSlug.includes("para"));
    } else if (targetTrack === "special") {
      candidatePool = candidatePool.filter((q) => q.id.startsWith("special-") || q.topicSlug.includes("dm") || q.topicSlug.includes("innovation"));
    }
  }
  ...
  if (targetDifficulty) {
    ...
    if (diffMatched.length >= 3) {
      candidatePool = diffMatched;
    }
  }

  // Shuffle the candidate questions to ensure every attempt gets different questions
